...
print(dataset, "\n\n\n", dataset['Review'], "\n\n\n", 
      sliced_dataset, "\n\n\n",
      len(dataset), len(sliced_dataset), np.shape(y_test), np.shape(X_test), np.shape(X_sliced_test),
      "\n\n\n",
      X_test, X_sliced_test)

acc_full_data, _ = evaluate_model(classifier_fulldata, X_test, y_test)
acc_data_slice, _ = evaluate_model(classifier_fulldata, X_train, y_train)
